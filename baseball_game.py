...
def get_not_duplicated_three_digit_number():
    ...
    while not is_validated_number(random_n) :
        random_n = get_random_number()
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = [0, 0]
    for i,s in enumerate(random_number):
        if s == user_input_number[i]:
            result[0] += 1
        elif s in user_input_number:
            result[1] += 1
    # ==================================
    return result
